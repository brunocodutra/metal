...
#define _boilerplate(M, N) \
    ...
    _assert((metal::is_just_t<metal::apply<_expr()<metal::_0> _comma(N) _vals(N)>>), (_true)); \
    _assert((metal::is_just_t<metal::apply<_expr()<metal::_0 _comma(M) _args(M)> _comma(N) _vals(N)>>), (_bool(M <= N))); \
    ...
    _assert((metal::apply_t<_expr()<metal::_0> _comma(M) _vals(M)>), (_expr()<_vals(M)>::type)); \
    _assert((metal::apply_t<_expr()<metal::_0 _comma(M) _args(M)> _comma(M) _vals(M)>), (_expr()<_vals(M) _comma(M) _vals(M)>::type)); \
