...
#define MATRIX(M, N) \
    ...
    ASSERT((metal::flatten_t<metal::fold_t<LIST(M), metal::list<>, metal::lambda<metal::list>>>), (metal::list<VALS(M)>)); \
    ASSERT((metal::flatten_t<metal::list<metal::reduce_t<LIST(INC(M)), metal::lambda<metal::list>>>>), (metal::list<VALS(INC(M))>)); \
    ASSERT((metal::slice_t<LIST(M), NUM(0), NUM(M)>), (LIST(M))); \
    ASSERT((metal::transform_t<metal::at<metal::quote_t<LIST(M)>, ARG(0)>, SEQ(M)<NUMS(M)>>), (LIST(M))); \
    ASSERT((metal::fold_t<VEC(M), metal::list<>, metal::push_front<metal::_1, metal::_2>, NUM(M), NUM(0)>), (metal::fold_t<VEC(M), metal::list<>, metal::push_back<metal::_1, metal::_2>>)); \
