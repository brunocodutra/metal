...
#define MATRIX(M, N) \
    ...
    CHECK((metal::is_invocable<metal::lambda<metal::pow>, NUM(M), INT(-N)>), (BOOL(M || !N))); \
    CHECK((metal::pow<NUM(M) COMMA(N) NUMS(N)>), (test::num<test::value_type<NUM(M) COMMA(N) NUMS(N) COMMA(N) IF(NOT(N))( , INT(0))>, IF(N)(1, M)>)); \
/**/
