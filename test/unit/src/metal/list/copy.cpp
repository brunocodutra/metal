...
#define MATRIX(M, N) \
    ...
    CHECK((metal::copy<PAIR(M), VALUE(N)>), (std::conditional_t<M == N, metal::list<VALUE(M)>, metal::list<>>)); \
    CHECK((metal::copy<PAIR(M), NUMBER(N)>), (std::conditional_t<M == N, metal::list<NUMBER(M)>, metal::list<>>)); \
