...
#define MATRIX(M, N) \
    ...
    CHECK((metal::remove<PAIR(M), VALUE(N)>), (metal::if_<BOOL(M == N), metal::list<NUMBER(M)>, PAIR(M)>)); \
    CHECK((metal::remove<PAIR(M), NUMBER(N)>), (metal::if_<BOOL(M == N), metal::list<VALUE(M)>, PAIR(M)>)); \
